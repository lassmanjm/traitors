...
# ----------------------------------------[ Murder ]----------------------------------------
    ...
    async def Murder(ctx: discord.Interaction, num_victims:int = 1):
        ...
        user_options = [discord.SelectOption(label=member.display_name, value=member.id) for member in await utils.GetFaithful()]
...
# ----------------------------------------[ Recruit ]----------------------------------------
    async def RecruitComplete():
        ...
        await control_channel.send(embed=discord.Embed(
    ...
    async def RecruitResponseCallback(interaction: discord.Interaction, view, selected_user: discord.User, force:bool):
    ...
    async def RecruitDecideCallback(interaction: discord.Interaction, view):
        ...
        if decision == "yes":
            ...
            await interaction.response.send_message(f"You have rejected the opportunity to recruit. Your numbers will not change.")
...
# ----------------------------------------[ Deathmatch ]----------------------------------------
    ...
    async def DeathMatch(ctx: discord.Interaction, num_players:int = 4, num_victims:int = 1):
        traitors_channel = await utils.TraitorsInstructionsChannel()
        ...
        user_options = [discord.SelectOption(label=member.display_name, value=member.id) for member in utils.GetPlayers()]
